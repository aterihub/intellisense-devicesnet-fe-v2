...
const header = [
  ...
  { text: "Created Time", value: "formattedCreatedAt", sortable: true },
  { text: "Updated Time", value: "formattedUpdatedAt", sortable: true },
...
const { tenants, isLoading, status, createTenantLoading, editTenantLoading } = storeToRefs(useTenantsStore())
...
async function deleteItem() {
  ...
  await tenantsStore.deleteTenant(selectedItem.value.id)
...
const onEdit = async (values, { resetForm }) => {
  ...
  if (submitClicked === 2) {
    loadingStore.startLoading()
    await tenantsStore.editTenant(selectedEditItem.value.id, newValues)
    loadingStore.stopLoading()
    isEdit.value = false
    submitLabel = 'Submit'
    submitClicked = 0
...
<template>
  ...
  <div class="grid grid-cols-3 px-8 gap-8 flex-1">
    <div class="flex flex-col gap-4 col-span-2">
      ...
      <EasyDataTable :rows-per-page="10" table-class-name="customize-table" :headers="header" :items="tenants"
      ...
      <VeeForm :validation-schema="schema" v-slot="{ handleSubmit }" as="div" ref="form">
        <form @submit="handleSubmit($event, onSubmit)" class="form-wrapper">
          ...
          <div class="flex justify-between gap-10">
            <BaseButton type="submit" class="primary" :label="submitLabel" :loading="createTenantLoading" />
          </div>
        </form>
      </VeeForm>
    </div>
    <div class="rounded-lg border-2 border-bkg-secondary px-4 py-6 flex flex-col gap-8" v-if="isEdit">
      ...
      <VeeForm :validation-schema="schema" v-slot="{ handleSubmit }" as="div" ref="form">
        <form @submit="handleSubmit($event, onEdit)" class="form-wrapper">
          <BaseInput v-model="tenantName" name="name" type="text" placeholder="Name" label="Name" />
          <BaseInput v-model="description" name="description" type="text" placeholder="Description" label="Description" />
          <div class="flex justify-between gap-10">
            <BaseButton type="button" class="secondary" label="Cancel" :loading="editTenantLoading" @click="cancelEdit" />
            <BaseButton type="submit" class="primary" :label="submitLabel" :loading="editTenantLoading" />
          </div>
        </form>
      </VeeForm>
    </div>
